<?php
if (!class_exists("MIUWPWPFUploadFile")) {
    class MIUWPWPFUploadFile extends WPForms_Field
    {

        public function init()
        {
            $this->name = esc_html__('Upload Image', 'default');
            $this->type = 'upload-image';
            $this->icon = 'fa-cloud-upload';
            $this->order = 101;

            add_filter(
            	'wpforms_field_properties_text',
	            array( $this, 'field_properties' ),
	            5, 3
            );
        }
        ...
        public function field_preview( $field )
        {
            ...
            echo "<div style='width: 100%; height: 100px; border: 1px dashed #bfbfbf; display: flex; justify-content: center; align-items: center;background: #f1f0ef;'>
					" . esc_html__('Drag & Drop your files or Browse', 'default') ."
				</div>";
        ...
        public function field_display( $field, $field_atts, $form_data )
        {
            ...
            $class = $primary['class'];
            $class[] = ' jquery-file-upload';
            $max_file_size = isset($field['max_file_size']) ? $field['max_file_size'] : "";
            ...
            printf("<div id='smfw-alert'></div>");

            printf(
                "<input type='file' %s %s %s %s %s %s target-to='%s'>",
            ...
            printf(
                "<input type='hidden' name='%s' class='%s' id='%s' />",
                $primary['attr']['name'] . "[file_id]",
                "upload-file-id",
                $primary['id'] . "-file"
            );
            printf(
                "<input type='hidden' class='wpf-attributes' file-type='%s' file-amount='%s' file-size='%s' >",
                $field['types'],
                $field['amount'],
                $file_size
            );
        ...
        public function field_option( $option, $field, $args = array(), $echo = true )
        {
            ...
            case 'amount':
                ...
                $output  = $this->field_element('label', $field, array( 'slug' => 'amount', 'value' => esc_html__('Max Allowed Files', 'wpforms'), 'tooltip' => $tooltip ), false);
            ...
            case 'types':
                ...
                $tooltip = esc_html__('Enter file types comma (,) separated: for example (jpeg, png)', 'wpf-drop-uploader');
        ...
        protected function convert_type_to_mime( $type )
        {
            $return_mime = "";
            $file_ext_mime = [
            'jpg' => 'image/jpeg',
